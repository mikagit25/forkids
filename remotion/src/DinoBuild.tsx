 ...
 * Silhouette fades in → 5 parts fly on arc trajectories → magnetic spring snap → celebration.
 * No text. Universal content.
 *
 * Timeline per dino (420f = 14s at 30fps):
 *   0–60f    INTRO     — silhouette ghost fades in, parallax bg settles
 *   60–120f  BODY      — body part flies in and snaps
 *   120–180f HEAD      — head flies in and snaps
 *   180–240f LEGS      — legs fly in and snap
 *   240–300f TAIL      — tail flies in and snaps
 *   300–420f CELEBRATE — all parts bounce, confetti, screen flash
 *
 * 3 dino color themes cycle. Total: 3 × 420 = 1260f loop. Composition: 1800f.
 */
...
// ── Timing ────────────────────────────────────────────────────────────────────
const INTRO_DUR    = 60;
const PART_DUR     = 60;  // each piece flies in over 60f (45f arc + 15f snap)
const PART_FLY     = 45;
const SNAP_DUR     = 15;
const CELEBRATE_DUR = 120;
const PARTS_COUNT  = 4;   // body, head, legs, tail
const BUILD_DUR    = INTRO_DUR + PARTS_COUNT * PART_DUR; // 300f
const CYCLE        = BUILD_DUR + CELEBRATE_DUR;           // 420f

// ── Dino themes ───────────────────────────────────────────────────────────────
interface DinoCfg {
  sprite: string;        // used for all parts (tinted via CSS hue-rotate)
  hueRotate: number;     // CSS hue-rotate degrees to tint sprite
  bgTop: string;
  bgBottom: string;
  accentColor: string;
  snapColor: string;
...
const DINOS: DinoCfg[] = [
  {
    sprite: "sprites/animals/dino.png",
    hueRotate: 0,
    bgTop:    "#1B5E20",
    bgBottom: "#4CAF50",
    accentColor: "#76FF03",
...
// ── Part config (positions relative to center) ────────────────────────────────
interface PartCfg {
  label: string;
  targetX: number;
  targetY: number;
...
// ── Screen shake (on each snap) ───────────────────────────────────────────────
function getShake(frame: number, snapFrames: number[]): { x: number; y: number } {
  for (const sf of snapFrames) {
    const rel = frame - sf;
    if (rel >= 0 && rel < 8) {
      const amp = interpolate(rel, [0, 8], [10, 0], { extrapolateRight: "clamp" });
      return {
        x: Math.sin(rel * 3.5) * amp,
        y: Math.cos(rel * 2.8) * amp * 0.6,
      };
    }
  }
  return { x: 0, y: 0 };
    ...
    {Array.from({ length: 36 }, (_, i) => {
      ...
      return (
        <div key={i} style={{
          ...
          opacity: op,
          transform: `rotate(${frame * 7 + i * 37}deg)`,
          pointerEvents: "none",
        }} />
      );
    })}
...
export const DinoBuild: React.FC<DinoBuildProps> = ({
  ...
  const totalCycle = DINOS.length * CYCLE;
  ...
  const isBuild     = localF >= INTRO_DUR && localF < BUILD_DUR;
  ...
  const victorySp = spring({
    frame: celebrateF,
    fps,
    config: { damping: 7, stiffness: 110, mass: 0.5 },
    durationInFrames: 25,
  });
  const victoryY = isCelebrate
    ? interpolate(victorySp, [0, 1], [0, -100], { extrapolateRight: "clamp" })
    : 0;
  const victoryScale = isCelebrate
    ? 1 + Math.abs(Math.sin(celebrateF * 0.22)) * 0.1
    : 1;

  // Collect snap frame times for screen shake
  const snapFrames = PARTS.map((_, pi) => INTRO_DUR + pi * PART_DUR + PART_FLY);
  const shake = getShake(localF, snapFrames);
  ...
  return (
    <AbsoluteFill style={{
      ...
      {PARTS.map((part, pi) => {
        ...
        let px  = cx + part.targetX;
        let py  = cy + part.targetY;
        let sc  = 1;
        ...
        if (isFlying) {
          ...
          const arcY = -part.arcHeight * 4 * t * (1 - t);
          py = (cy + part.fromY) + ease * ((cy + part.targetY) - (cy + part.fromY)) + arcY;
          sc  = 0.6 + ease * 0.4;
          rot = part.rotation * ease;
        } else if (isSnapping) {
          const snapSp = spring({
            frame: partLocalF - PART_FLY,
            fps,
            config: { damping: 5, stiffness: 220, mass: 0.25 },
            durationInFrames: SNAP_DUR,
          });
          sc = 1 + Math.abs(snapSp - 1) * 0.22;
        }

        if (hasLanded && isCelebrate) {
          py += victoryY;
          sc  *= victoryScale;
        }
      ...
      {/* Victory flash on first frame of celebration */}
