#!/usr/bin/env python3
"""
Generate Bubble Pop Song videos.
Scenario: config/scenarios/bubble_pop_song_*.txt
Usage:
...
def main():
    ...
    scenario_files = list(SCENARIO_DIR.glob(f"bubble_pop_song_{args.key}*.txt"))
    if scenario_files:
    ...
    if is_no_text:
        ...
        for lg in langs:
            ...
            if not meta_path.exists() or args.regen_meta:
                meta = make_meta(args.key, lg, "Bubble Pop Song", "أغنية فرقعة الفقاعات", "Lagu Pop Gelembung")
                if not args.dry_run:
        ...
        for lg in langs:
            q = queues[lg]
            out_mp4 = q / f"bubble_pop_song_{args.key}_{lg}_{DATE_STR}.mp4"
            if not out_mp4.exists() and not args.dry_run and not args.regen_meta:
                cmd = ["npx", "remotion", "render", "NurseryRhymeLong",
                       f"--props={json.dumps(props)}", f"--output={str(out_mp4)}"]
                r = subprocess.run(cmd, cwd=str(REMOTION), timeout=86400)
